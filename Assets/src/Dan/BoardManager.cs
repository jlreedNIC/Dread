...
public class BoardManager : MonoBehaviour
{
   ...
   [Serializable]
   // Count class sets up serialized fields to have a min and max value for
   // level dimensions and is reused for interior wall parameters
   public class Count
   {
      public int minimum;
      public int maximum;
   ...
   // initialize gameboard size is set to default values for an 8 x 8 2D array
...
=======
   ...
   public void SetupScene(int level)
   {
      ...
      if (columns < 8 || rows < 8)
      {
         ...
         // place all tiles and walls in the scene
         LayoutObjectAtRandom(wallTiles, interiorWallCount.minimum, interiorWallCount.maximum);
      /* [Place holder For Any Enemy instantiation if needed]
       * Spawn # of enemies based on the level's number IE: level 1 spawn 1 enemy; 2 spawn 2,..etc.
       * int enemyCount = (int)Mathf.Log(level, 2f);
